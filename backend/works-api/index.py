...
Поддерживает: получение списка, загрузку новой работы, модерацию.
"""
import json
import os
import base64
...
def handler(event: dict, context) -> dict:
    ...
    # GET /works?region=north  — список одобренных работ
    if method == 'GET' and path == '/':
        ...
        if region:
            cur.execute(
                f"SELECT id, region_key, title, author_name, category, description, image_url, created_at "
                f"FROM {SCHEMA}.works WHERE status='approved' AND region_key=%s ORDER BY created_at DESC",
                (region,)
            )
        else:
            cur.execute(
                f"SELECT id, region_key, title, author_name, category, description, image_url, created_at "
                f"FROM {SCHEMA}.works WHERE status='approved' ORDER BY created_at DESC"
            )
        rows = cur.fetchall()
        conn.close()
        cols = ['id', 'region_key', 'title', 'author_name', 'category', 'description', 'image_url', 'created_at']
        return json_response([dict(zip(cols, r)) for r in rows])

    # GET /?action=regions — список регионов
    ...
    # POST / — загрузить новую работу
    if method == 'POST' and path == '/':
        required = ['title', 'author_name', 'category', 'region_key', 'image_base64', 'image_ext']
        for f in required:
            if not body.get(f):
        ...
        cur = conn.cursor()
        cur.execute(
            f"INSERT INTO {SCHEMA}.works (region_key, title, author_name, category, description, image_url, status) "
            f"VALUES (%s, %s, %s, %s, %s, %s, 'pending') RETURNING id",
            (body['region_key'], body['title'], body['author_name'], category,
